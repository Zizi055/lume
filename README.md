...
Один раз нужно включить публикацию: **Settings → Pages → Source → GitHub Actions**.
Без этого Pages отдаёт исходники репозитория, а не собранную версию, и страница будет пустой.

Пути к ассетам относительные (`base: './'`), поэтому сборка одинаково работает
и в корне домена, и в подпапке вида `/lume/`.
